...
class GoalInferenceTests:
    def __init__(self, goal_a, goal_b, goal_c, goal_d, goal_e, rosbag_file_name, test_scenario, trial):
        self.goal_a = goal_a
        self.goal_b = goal_b
        self.goal_c = goal_c
        self.goal_d = goal_d
        ...
        print("Goal d:\nx: {}\ny: {}".format(goal_d.x, goal_d.y))
        print("Goal e:\nx: {}\ny: {}".format(goal_e.x, goal_e.y))
        ...
        self.model = joblib.load(os.path.join(rospack.get_path(
            "detect_poi"), "ml_models/goal_inference_5_goals.sav"))
    ...
    def calculate_distance_from_each_goal(self, odometry_msg):
        ...
        distances.append(sqrt((self.goal_d.x - odometry_msg.pose.pose.position.x)**2 + (self.goal_d.y - odometry_msg.pose.pose.position.y)**2)*1000)
        distances.append(sqrt((self.goal_e.x - odometry_msg.pose.pose.position.x)**2 + (self.goal_e.y - odometry_msg.pose.pose.position.y)**2)*1000)
        return distances
    ...
    def calculate_angle_from_each_goal(self, odometry_msg):
        ...
        # For goal_d
        marker_x_difference = self.goal_d.x - odometry_msg.pose.pose.position.x
        marker_y_difference = self.goal_d.y - odometry_msg.pose.pose.position.y
        ...
        # For goal_e
        marker_x_difference = self.goal_e.x - odometry_msg.pose.pose.position.x
        marker_y_difference = self.goal_e.y - odometry_msg.pose.pose.position.y
    ...
    def calculate_approaching_speed_towards_each_goal(self, odometry_msg):
        ...
        #For goal_d
        direction_vector.x = self.goal_d.x - odometry_msg.pose.pose.position.x
        direction_vector.y = self.goal_d.y - odometry_msg.pose.pose.position.y
        ...
        #For goal_e
        direction_vector.x = self.goal_e.x - odometry_msg.pose.pose.position.x
        direction_vector.y = self.goal_e.y - odometry_msg.pose.pose.position.y
    ...
    def callback(self, odometry_msg):
        ...
        goal_list = ["goal_a", "goal_b", "goal_c", "goal_d", "goal_e"]
...
if __name__ == "__main__":
    ...
    if len(sys.argv) >= 2:
        ...
        # Goal d
        goal_d = Point()
        goal_d.x = json_content["test_scenarios"][test_scenario]["goal_d"]["x"]
        goal_d.y = json_content["test_scenarios"][test_scenario]["goal_d"]["y"]

        # Goal e
        goal_e = Point()
        goal_e.x = json_content["test_scenarios"][test_scenario]["goal_e"]["x"]
        goal_e.y = json_content["test_scenarios"][test_scenario]["goal_e"]["y"]

        try:
            gi = GoalInferenceTests(goal_a, goal_b, goal_c, goal_d, goal_e, rosbag_file_name, test_scenario, trial)
