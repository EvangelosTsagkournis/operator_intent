...
//  }
  ...
  void imageCallback(const sensor_msgs::ImageConstPtr& msg)
  {
    ...
    */
    ...
    // The code below will attempt to draw a bounding box containing the tag, if tags were detected successfully
    if (markerCorners.size() > 0) {
      ...
      for (unsigned long int i = 0; i < orthogonalMarkerCorners.size(); i++){
        ...
        // Check for the minimum and maximum values of x and y of all the corners, in order to draw a orthogonal bounding box.
        ...
        orthogonalMarkerCorners[i][0].x = min_x;
        orthogonalMarkerCorners[i][0].y = min_y;
        orthogonalMarkerCorners[i][1].x = max_x;
        orthogonalMarkerCorners[i][1].y = min_y;
        orthogonalMarkerCorners[i][2].x = max_x;
        orthogonalMarkerCorners[i][2].y = max_y;
        orthogonalMarkerCorners[i][3].x = min_x;
        orthogonalMarkerCorners[i][3].y = max_y;

        int thickness = 1;
        // Loop through the orthogonalMarkerCorners and draw the bounding boxes

        for (unsigned int j = 0; j < 3; j++){
          cv::line(cv_ptr->image, orthogonalMarkerCorners[i][j], orthogonalMarkerCorners[i][j+1], cv::Scalar(0, 255, 0), thickness, cv::LINE_8);
        }
        cv::line(cv_ptr->image, orthogonalMarkerCorners[i][3], orthogonalMarkerCorners[i][0], cv::Scalar(0, 255, 0), thickness, cv::LINE_8);
      }
    }
